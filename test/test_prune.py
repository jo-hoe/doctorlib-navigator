...
def test_immediate_strategy_keeps_only_present():
    strategy = ImmediatePruneStrategy()
    last_seen = {"a": _at(0), "b": _at(5)}
    assert strategy.live_slots(last_seen, _at(10)) == {"a", "b"}
...
def test_ttl_strategy_keeps_within_ttl():
    ...
    assert strategy.live_slots(last_seen, _at(11)) == {"fresh"}
...
def test_ttl_store_absorbs_jitter(tmp_path: Path):
    ...
    store.save("k", frozenset(["slot"]))  # seen at 12:00
    now["t"] = _at(5)
    store.save("k", frozenset())  # vanished at 12:05, still within TTL
    # load reflects that the slot is still considered present
...
def test_ttl_store_evicts_after_ttl(tmp_path: Path):
    ...
    now["t"] = _at(11)  # 12:11 → last_seen 12:00 older than 10m TTL
...
def test_ttl_store_reappearance_refreshes_timestamp(tmp_path: Path):
    ...
    now["t"] = _at(8)
    store.save("k", frozenset(["slot"]))  # reappeared at 12:08, refreshes to 12:08
    now["t"] = _at(15)  # 12:15, only 7m since refresh → still live
    assert store.load("k") == frozenset(["slot"])
